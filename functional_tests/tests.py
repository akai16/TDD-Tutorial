...
class NewVisitorTest(LiveServerTestCase):
  ...
  def test_multiple_users_can_start_lists_at_different_urls(self):
    ...
    iara_list_url = self.browser.current_url
    self.assertRegex(iara_list_url, r'/lists/.+')


    # Now a new user, Chicão, comes along to the site

    ## We use a new browser session to make sure that
    ## no information of Iara's is coming through from cookies etc
    self.browser.quit()
    self.browser = webdriver.Firefox()
    self.browser.get(self.live_server_url)
    ...
    # Chicão gets his own unique URL
    chicao_list_url = self.browser.current_url
    self.assertRegex(chicao_list_url, r'/lists/.+')
    self.assertNotEqual(iara_list_url, chicao_list_url)

    # Again, there is no trace of Iara's list
    page_text = self.browser.find_element_by_tag_name('body').text
    self.assertNotIn('Buy peacock feathers', page_text)
  ...
  def test_layout_and_styling(self):
    ...
    inputbox = self.browser.find_element_by_id('id_new_item')
    self.assertAlmostEqual(
      inputbox.location['x'] + inputbox['size']/2,
      512,
      delta=10
    )