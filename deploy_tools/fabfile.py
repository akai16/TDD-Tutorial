...
REPO_URL = 'https://github.com/akai16/TDD-Tutorial'

def deploy():
    site_folder = f'/home/{env.user}/sites/{env.host}'
    source_folder = f'{site_folder}/src'
...
def _create_directory_structure_if_necessary(site_folder):
    for subfolder in ('database', 'static', 'venv', 'src'):
...
def _update_settings(source_folder, site_name):
    ...
    append(settings_path, f'from .secret_key import SECRET_KEY')


def _update_virtualenv(source_folder):
    virtualenv_folder = f'{source_folder}/../venv'
    if not exists('{virtualenv_folder}/bin/pip'):
        run(f"python3 -m virtualenv {virtualenv_folder}")
